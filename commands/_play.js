...
function getTitle(ytid){
    request(
        `https://www.googleapis.com/youtube/v3/videos?id=${id}&key=${config["ytapikey"]}&fields=items(snippet(title))&part=snippet`,
            ...
            if (message.member.voiceChannel) {
                ...
                if (config["queue"].length > 0 || config["Playing"]) {
                    ...
                    if (query.type === "yt")
                        ...
                        request(
                            `https://www.googleapis.com/youtube/v3/search?part=id&type=video&q=${encodeURIComponent(query)}&key=${config["ytapikey"]}`,
                            function(error, response, body) {
                                var json = JSON.parse(body);
                                if (json.items[0]) {
